...
class TypeChecker:
    ...
    def check(self, program: ast.Program) -> CheckResult:
        ...
        for stmt in program.statements:
            if isinstance(stmt, ast.FuncDecl):
                if stmt.name in self.function_signatures:
                    raise YasnError(f"Функция '{stmt.name}' уже объявлена", stmt.line, stmt.col, self.path)
        ...
        for stmt in program.statements:
            ...
            if self._check_stmt(stmt, current_return_type=None):
                raise YasnError("Возврат из функции вне контекста функции", stmt.line, stmt.col, self.path)
        ...
        if "main" in self.function_signatures:
            ...
            if main_sig.params:
                raise YasnError("Функция main должна быть без параметров", line, col, self.path)
            if main_sig.return_type != VOID:
                raise YasnError("Функция main должна возвращать Пусто", line, col, self.path)
            if main_sig.is_async:
                raise YasnError("Функция main не может быть асинхронной", line, col, self.path)
    ...
    def _check_stmt(self, stmt: ast.Stmt, current_return_type: Type | None) -> bool:
        ...
        if isinstance(stmt, ast.IfStmt):
            ...
            if cond_t != BOOL:
                raise YasnError("Условие 'если' должно иметь тип Лог", stmt.condition.line, stmt.condition.col, self.path)
        ...
        if isinstance(stmt, ast.WhileStmt):
            cond_t = self._check_expr(stmt.condition)
            if cond_t != BOOL:
                raise YasnError("Условие 'пока' должно иметь тип Лог", stmt.condition.line, stmt.condition.col, self.path)
            self.loop_depth += 1
            self._push_scope()
        ...
        if isinstance(stmt, ast.BreakStmt):
            if self.loop_depth <= 0:
                raise YasnError("'прервать' допустим только внутри цикла", stmt.line, stmt.col, self.path)
            return False

        if isinstance(stmt, ast.ContinueStmt):
            if self.loop_depth <= 0:
                raise YasnError("'продолжить' допустим только внутри цикла", stmt.line, stmt.col, self.path)
            return False

        if isinstance(stmt, ast.ReturnStmt):
            if current_return_type is None:
                raise YasnError("Оператор 'вернуть' разрешён только внутри функции", stmt.line, stmt.col, self.path)
            value_t = self._check_expr(stmt.value)
        ...
        if isinstance(stmt, ast.FuncDecl):
            raise YasnError("Вложенные объявления функций не поддерживаются", stmt.line, stmt.col, self.path)

        raise YasnError("Неизвестный тип оператора", stmt.line, stmt.col, self.path)
    ...
    def _check_expr(self, expr: ast.Expr) -> Type:
        if isinstance(expr, ast.Literal):
            ...
            if expr.kind == "null":
                ...
                return VOID
            raise YasnError("Неизвестный литерал", expr.line, expr.col, self.path)
        ...
        if isinstance(expr, ast.UnaryOp):
            ...
            if expr.op == "не":
                if t != BOOL:
                    raise YasnError("Оператор 'не' принимает только Лог", expr.line, expr.col, self.path)
                expr.inferred_type = BOOL
                return BOOL
            if expr.op == "-":
                if not is_numeric_like(t):
                    raise YasnError("Унарный '-' принимает только Цел/Дроб", expr.line, expr.col, self.path)
                expr.inferred_type = t
                return t
            raise YasnError(f"Неизвестный унарный оператор: {expr.op}", expr.line, expr.col, self.path)

        if isinstance(expr, ast.AwaitExpr):
            operand_t = self._check_expr(expr.operand)
            if not is_assignable(TASK, operand_t):
                raise YasnError("Оператор 'ждать' принимает только Задача", expr.line, expr.col, self.path)
        ...
        if isinstance(expr, ast.BinaryOp):
            ...
            if expr.op in {"и", "или"}:
                if left_t != BOOL or right_t != BOOL:
                    raise YasnError("Операторы 'и/или' требуют Лог", expr.line, expr.col, self.path)
                expr.inferred_type = BOOL
                return BOOL

            raise YasnError(f"Неизвестный бинарный оператор: {expr.op}", expr.line, expr.col, self.path)
        ...
        if isinstance(expr, ast.Call):
            if not isinstance(expr.callee, ast.Identifier):
                raise YasnError("Вызов возможен только по имени функции", expr.line, expr.col, self.path)
            fn_name = expr.callee.name
            arg_types = [self._check_expr(arg) for arg in expr.args]
            if fn_name not in self.function_signatures:
                raise YasnError(f"Неизвестная функция: {fn_name}", expr.line, expr.col, self.path)
        ...
        raise YasnError("Неизвестный тип выражения", expr.line, expr.col, self.path)
        ...
        for variant in variants_of(target_t):
            if variant.name == "Список":
                if not is_assignable(INT, index_t):
                    raise YasnError("Индекс списка должен иметь тип Цел", line, col, self.path)
            ...
            if variant == STRING:
                if not is_assignable(INT, index_t):
                    raise YasnError("Индекс строки должен иметь тип Цел", line, col, self.path)
                result_types.append(STRING)
                continue
            raise YasnError("Индексирование поддерживается только для Список/Словарь/Строка", line, col, self.path)
    ...
    def _check_builtin_call(self, expr: ast.Call, name: str, arg_types: list[Type]) -> Type:
        ...
        if name == "длина":
            if len(arg_types) != 1:
                raise YasnError("длина(x) принимает ровно 1 аргумент", expr.line, expr.col, self.path)
            arg_t = arg_types[0]
            for variant in variants_of(arg_t):
                if variant != STRING and variant.name != "Список":
                    raise YasnError("длина(x) поддерживает только Строка или Список[Т]", expr.line, expr.col, self.path)
            return INT
        if name == "диапазон":
            if len(arg_types) != 2:
                raise YasnError("диапазон(нач, конец) принимает 2 аргумента", expr.line, expr.col, self.path)
            if not is_assignable(INT, arg_types[0]) or not is_assignable(INT, arg_types[1]):
                raise YasnError("диапазон(нач, конец) принимает только Цел", expr.line, expr.col, self.path)
            return list_of(INT)
        if name == "ввод":
            if arg_types:
                raise YasnError("ввод() не принимает аргументы", expr.line, expr.col, self.path)
            return STRING
        if name == "пауза":
            if len(arg_types) != 1:
                raise YasnError("пауза(мс) принимает ровно 1 аргумент", expr.line, expr.col, self.path)
            if not is_assignable(INT, arg_types[0]):
                raise YasnError("пауза(мс) принимает только Цел", expr.line, expr.col, self.path)
            return VOID
        if name == "строка":
            if len(arg_types) != 1:
                raise YasnError("строка(x) принимает ровно 1 аргумент", expr.line, expr.col, self.path)
            return STRING
        if name == "число":
            if len(arg_types) != 1:
        ...
        if name == "запустить":
            ...
            if not is_assignable(STRING, arg_types[0]):
                raise YasnError("Первый аргумент запустить(...) должен быть Строка", expr.line, expr.col, self.path)
            return TASK
        if name == "готово":
            if len(arg_types) != 1:
                raise YasnError("готово(задача) принимает ровно 1 аргумент", expr.line, expr.col, self.path)
            if not is_assignable(TASK, arg_types[0]):
                raise YasnError("готово(задача) принимает только Задача", expr.line, expr.col, self.path)
            return BOOL
        if name == "ожидать":
            if len(arg_types) not in (1, 2):
                raise YasnError("ожидать(задача[, таймаут_мс]) принимает 1 или 2 аргумента", expr.line, expr.col, self.path)
            if not is_assignable(TASK, arg_types[0]):
                raise YasnError("Первый аргумент ожидать(...) должен быть Задача", expr.line, expr.col, self.path)
            if len(arg_types) == 2 and not is_assignable(INT, arg_types[1]):
                raise YasnError("Второй аргумент ожидать(...) должен быть Цел", expr.line, expr.col, self.path)
            return ANY
        if name == "ожидать_все":
            if len(arg_types) not in (1, 2):
                raise YasnError(
            ...
            if len(arg_types) == 2 and not is_assignable(INT, arg_types[1]):
                raise YasnError("Второй аргумент ожидать_все(...) должен быть Цел", expr.line, expr.col, self.path)
            return list_of(ANY)
        if name == "отменить":
            if len(arg_types) != 1:
                raise YasnError("отменить(задача) принимает ровно 1 аргумент", expr.line, expr.col, self.path)
            if not is_assignable(TASK, arg_types[0]):
                raise YasnError("отменить(задача) принимает только Задача", expr.line, expr.col, self.path)
            return BOOL
        raise YasnError(f"Неизвестная встроенная функция: {name}", expr.line, expr.col, self.path)
    ...
    def _define_var(self, name: str, t: Type, line: int, col: int) -> None:
        scope = self.scopes[-1]
        if name in scope:
            raise YasnError(f"Переменная '{name}' уже объявлена в этой области", line, col, self.path)
        scope[name] = t
    ...
    def _resolve_var(self, name: str, line: int, col: int) -> Type:
        for scope in reversed(self.scopes):
            if name in scope:
                return scope[name]
        raise YasnError(f"Неизвестная переменная: {name}", line, col, self.path)
