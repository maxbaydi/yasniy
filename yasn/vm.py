...
class VirtualMachine:
    ...
    def _call_function(self, function_name: str, args: list[object], globals_store: list[object]) -> object:
        ...
        if function_name not in self.program.functions:
            raise YasnError(f"Неизвестная функция: {function_name}", path=self.path)
        ...
        while frame.ip < len(instructions):
            ...
            if op == "CONST":
                ...
                if isinstance(target, (list, str)):
                    ...
                    raise YasnError(f"INDEX_GET не поддерживается для типа {type(target).__name__}", path=self.path)
            elif op == "INDEX_SET":
                value = stack.pop()
                idx = stack.pop()
                target = stack.pop()
                if isinstance(target, list):
                    ...
                    raise YasnError(f"INDEX_SET не поддерживается для типа {type(target).__name__}", path=self.path)
                stack.append(value)
            elif op == "LEN":
                value = stack.pop()
                ...
                raise YasnError(f"Неизвестная инструкция VM: {op}", path=self.path)
    ...
    def _builtin_len(self, args: list[object], _: list[object]) -> object:
        if len(args) != 1:
            raise YasnError("длина(x) принимает ровно 1 аргумент", path=self.path)
        return len(args[0])

    def _builtin_range(self, args: list[object], _: list[object]) -> object:
        if len(args) != 2:
            raise YasnError("диапазон(нач, конец) принимает 2 аргумента", path=self.path)
        start, end = args
        return list(range(int(start), int(end)))

    def _builtin_input(self, args: list[object], _: list[object]) -> object:
        if args:
            raise YasnError("ввод() не принимает аргументы", path=self.path)
        return input()

    def _builtin_sleep(self, args: list[object], _: list[object]) -> object:
        if len(args) != 1:
            raise YasnError("пауза(мс) принимает ровно 1 аргумент", path=self.path)
    ...
    def _builtin_to_string(self, args: list[object], _: list[object]) -> object:
        if len(args) != 1:
            raise YasnError("строка(x) принимает ровно 1 аргумент", path=self.path)
        return str(self._format_value(args[0]))

    def _builtin_to_int(self, args: list[object], _: list[object]) -> object:
        if len(args) != 1:
            raise YasnError("число(x) принимает ровно 1 аргумент", path=self.path)
        val = args[0]
    ...
    def _builtin_spawn(self, args: list[object], globals_store: list[object]) -> object:
        if len(args) < 1:
            raise YasnError("запустить(имя, ...args) требует минимум 1 аргумент", path=self.path)
        fn_name_raw = args[0]
        if not isinstance(fn_name_raw, str) or not fn_name_raw:
            raise YasnError("Первый аргумент запустить(...) должен быть непустой Строка", path=self.path)
        fn_name = fn_name_raw
        if fn_name not in self.builtins and fn_name not in self.program.functions:
            raise YasnError(f"Неизвестная функция: {fn_name}", path=self.path)
    ...
    def _builtin_done(self, args: list[object], _: list[object]) -> object:
        if len(args) != 1:
            raise YasnError("готово(задача) принимает ровно 1 аргумент", path=self.path)
    ...
    def _builtin_wait(self, args: list[object], _: list[object]) -> object:
        if len(args) not in (1, 2):
            raise YasnError("ожидать(задача[, таймаут_мс]) принимает 1 или 2 аргумента", path=self.path)
    ...
    def _builtin_wait_all(self, args: list[object], _: list[object]) -> object:
        ...
        if not isinstance(raw_tasks, list):
            raise YasnError("Первый аргумент ожидать_все(...) должен быть списком", path=self.path)
        timeout_s: float | None = None
        if len(args) == 2:
    ...
    def _builtin_cancel(self, args: list[object], _: list[object]) -> object:
        if len(args) != 1:
            raise YasnError("отменить(задача) принимает ровно 1 аргумент", path=self.path)
    ...
    def _coerce_non_negative_int(self, value: object, context: str) -> int:
        if isinstance(value, bool) or not isinstance(value, int):
            raise YasnError(f"{context}: ожидался Цел >= 0", path=self.path)
        if value < 0:
            raise YasnError(f"{context}: ожидался Цел >= 0", path=self.path)
        return value
    ...
    def _expect_task_handle(self, value: object, context: str) -> TaskHandle:
        if isinstance(value, TaskHandle):
            return value
        raise YasnError(f"{context}: ожидался объект типа Задача", path=self.path)

    def _wait_task_result(self, task: TaskHandle, timeout_s: float | None) -> object:
        try:
            return task.future.result(timeout=timeout_s)
        except CancelledError as exc:
            raise YasnError(f"Задача #{task.task_id} была отменена", path=self.path) from exc
        except TimeoutError as exc:
            raise YasnError(f"Истёк таймаут ожидания задачи #{task.task_id}", path=self.path) from exc
        except YasnError:
            raise
        except Exception as exc:  # noqa: BLE001
            raise YasnError(f"Задача #{task.task_id} завершилась с ошибкой: {exc}", path=self.path) from exc
