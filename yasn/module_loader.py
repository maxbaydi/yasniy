...
class ModuleResolver:
    ...
    def _load_config(self, path: Path) -> ModuleConfig:
        ...
        try:
            ...
            raise YasnError(f"РќРµ СѓРґР°Р»РѕСЃСЊ РїСЂРѕС‡РёС‚Р°С‚СЊ yasn.toml: {exc}", path=str(path)) from exc

        modules = data.get("modules", {})
        if not isinstance(modules, dict):
            raise YasnError("РЎРµРєС†РёСЏ [modules] РІ yasn.toml РґРѕР»Р¶РЅР° Р±С‹С‚СЊ РѕР±СЉРµРєС‚РѕРј", path=str(path))
        ...
        if "root" in modules:
            ...
            if not isinstance(root_val, str):
                raise YasnError("modules.root РґРѕР»Р¶РµРЅ Р±С‹С‚СЊ СЃС‚СЂРѕРєРѕР№", path=str(path))
            root = root_val
        if "paths" in modules:
            paths_val = modules["paths"]
            if not isinstance(paths_val, list) or not all(isinstance(x, str) for x in paths_val):
                raise YasnError("modules.paths РґРѕР»Р¶РµРЅ Р±С‹С‚СЊ СЃРїРёСЃРєРѕРј СЃС‚СЂРѕРє", path=str(path))
    ...
    def _resolve_module(self, module_path: Path, program: ast.Program | None, is_entry: bool) -> ResolvedModule:
        ...
        if module_path in self._resolving_stack:
            ...
            raise YasnError(f"РћР±РЅР°СЂСѓР¶РµРЅ С†РёРєР»РёС‡РµСЃРєРёР№ РёРјРїРѕСЂС‚: {chain}", path=str(module_path))
