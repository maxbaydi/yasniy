...
class _FunctionCompiler:
    ...
    def define_var(self, name: str, line: int, col: int) -> int:
        ...
        if name in current:
            raise YasnError(f"Переменная '{name}' уже объявлена в блоке", line, col, self.path)
    ...
    def compile_stmt(self, stmt: ast.Stmt) -> None:
        ...
        if isinstance(stmt, ast.FuncDecl):
            raise YasnError("Вложенные функции не поддерживаются", stmt.line, stmt.col, self.path)

        raise YasnError("Неизвестный узел оператора для компиляции", stmt.line, stmt.col, self.path)
    ...
    def compile_expr(self, expr: ast.Expr) -> None:
        ...
        raise YasnError("Неизвестный узел выражения для компиляции", expr.line, expr.col, self.path)
