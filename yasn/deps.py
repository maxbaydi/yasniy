...
def install_dependencies(cwd: Path | None = None, clean: bool = False) -> tuple[DependenciesManifest, list[DependencyInstallResult]]:
    ...
    while queue:
        ...
        if dep.spec.kind == "path":
            ...
            raise YasnError(f"Неподдерживаемый тип зависимости: {dep.spec.kind}", path=str(manifest.config_path))
...
def _find_project_context(start: Path) -> tuple[Path, Path]:
    ...
    for cur in search:
        ...
        if cfg is not None:
            return cur, cfg
    raise YasnError("Не найден yasn.toml. Зависимости требуют конфиг проекта.")
...
def _parse_dependencies(data: dict[str, Any], config_path: Path) -> list[DependencySpec]:
    ...
    if not isinstance(deps_raw, dict):
        raise YasnError("Секция [dependencies] должна быть объектом", path=str(config_path))

    specs: list[DependencySpec] = []
    seen: set[str] = set()
    for name, value in deps_raw.items():
        if not isinstance(name, str) or not name.strip():
            raise YasnError("Имя зависимости должно быть непустой строкой", path=str(config_path))
        dep_name = name.strip()
        if dep_name in seen:
            raise YasnError(f"Повтор имени зависимости: {dep_name}", path=str(config_path))
        seen.add(dep_name)
        ...
        if isinstance(value, str):
            ...
            if not isinstance(raw_source, str) or not raw_source.strip():
                raise YasnError(f"dependencies.{dep_name}.source должен быть непустой строкой", path=str(config_path))
            source = raw_source.strip()
            if raw_ref is not None:
                if not isinstance(raw_ref, str) or not raw_ref.strip():
                    raise YasnError(f"dependencies.{dep_name}.ref должен быть непустой строкой", path=str(config_path))
                ref = raw_ref.strip()
        else:
            raise YasnError(
...
def _normalize_source(source: str, ref: str | None, config_path: Path) -> tuple[str, str, str | None]:
    if source.startswith("git+"):
        raw = source[4:].strip()
        if not raw:
            raise YasnError("Пустой git-источник зависимости", path=str(config_path))
        if "#" in raw:
            ...
            raw = base.strip()
        if not raw:
            raise YasnError("Пустой URL git-источника зависимости", path=str(config_path))
        return "git", raw, ref

    if source.startswith("path:"):
        raw_path = source[5:].strip()
        if not raw_path:
            raise YasnError("Пустой path-источник зависимости", path=str(config_path))
...
def _install_from_path(spec: DependencySpec, project_root: Path, target: Path) -> tuple[str, Path]:
    ...
    if not source_path.exists():
        raise YasnError(f"Путь зависимости не найден: {source_path}")
    if not source_path.is_dir():
        raise YasnError(f"Путь зависимости должен быть директорией: {source_path}")
...
def _install_from_git(spec: DependencySpec, project_root: Path, target: Path) -> tuple[str, Path]:
    ...
    if _is_local_git_source(source):
        ...
        if not Path(source).exists():
            raise YasnError(f"Git-источник не найден: {source}")
...
def _run_cmd(cmd: list[str], cwd: Path) -> str:
    try:
        ...
        raise YasnError(f"Не найдено внешнее приложение: {cmd[0]}") from exc

    if proc.returncode != 0:
        stderr = (proc.stderr or "").strip()
        stdout = (proc.stdout or "").strip()
        details = stderr or stdout or "неизвестная ошибка"
        raise YasnError(f"Команда завершилась с ошибкой ({proc.returncode}): {' '.join(cmd)}\n{details}")
