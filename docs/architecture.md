...
- `асинхронная функция` и выражения `ждать`
- `если/иначе`, `пока`, `для`
- `прервать`, `продолжить`
- модульный синтаксис `подключить`, `из ... подключить`, `как`
- типы `Список/Словарь`, объединения `|`, nullable `?`, а также `Любой` и `Задача`
...
7. Ищет модули зависимостей в `.yasn/deps`

`yasn deps install` собирает граф зависимостей рекурсивно. Для масштабируемости
используется lock-файл `.yasn/deps.lock.json`, в который попадают и прямые, и транзитивные узлы.
...
- `main` не допускается как `асинхронная функция`
...
- выполняет builtins: `печать`, `длина`, `диапазон`, `ввод`, `пауза`, `строка`, `число`, `запустить`, `готово`, `ожидать`, `ожидать_все`, `отменить`
- запускает задачи через thread pool
- изолирует глобальное состояние для фоновых задач (snapshot на момент `запустить`)
- компилятор понижает вызовы `асинхронная функция ...` в внутренний `запустить(...)`
