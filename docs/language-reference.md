...
- Внутри `()`, `[]`, `{}` переводы строк и отступы не формируют `NEWLINE/INDENT/DEDENT`
...
- `Любой` — динамический тип: может быть присвоен в любую переменную, но ослабляет гарантии типизации
- `пусть x = expr` выводит тип из `expr`
...
- `main` не может быть асинхронной
- Вызов `асинхронная функция ...` возвращает `Задача`
...
4. Каталог зависимостей проекта `.yasn/deps` (если существует)
...
Зависимости устанавливаются командой `yasn deps` в каталог `.yasn/deps`.
Поддерживаются транзитивные зависимости; состояние фиксируется в `.yasn/deps.lock.json`.
...
- `асинхронная` функция при обычном вызове запускается в пуле потоков VM.
- `ждать task` эквивалентно `ожидать(task)`.
- `запустить` остаётся как низкоуровневый API запуска по имени функции.
- Каждая задача работает с изолированным снимком глобальных переменных на момент запуска.
- Изменения глобального состояния внутри задачи не применяются обратно в основной поток.
- Таймаут в `ожидать`/`ожидать_все` задаётся в миллисекундах.
- `отменить` выполняет best-effort отмену (успешно, если задача ещё не стартовала).
...
func_decl      = ["асинхронная"] "функция" ident "(" [params] ")" "->" type ":" newline
                 indent { stmt } dedent ;
params         = param { "," param } ;
param          = ident ":" type ;
...
prim_type      = "Цел" | "Дроб" | "Лог" | "Строка" | "Пусто" | "Любой" | "Задача" ;

expr           = or_expr ;
unary_expr     = ("не" | "-" | "ждать") unary_expr | postfix_expr ;
```
