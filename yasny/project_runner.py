...
def load_run_profile(
    ...
    if not isinstance(run_cfg, dict):
        raise YasnyError("Секция [run] должна быть объектом", path=str(config_path) if config_path else None)
    ...
    if not isinstance(mode_cfg_raw, dict):
        ...
        raise YasnyError(f"Секция [run.{mode}] должна быть объектом", path=cfg_path)
    ...
    if not backend_path:
        raise YasnyError(
            "Не найден backend entrypoint. Укажите [run].backend в yasn.toml/yasny.toml или передайте --backend"
        )
    ...
    try:
        ...
        raise YasnyError(f"Некорректный порт: {port_raw}") from exc
    ...
    if frontend_cmd is not None and not isinstance(frontend_cmd, str):
        raise YasnyError(f"run.{mode}.frontend должен быть строкой")
    ...
    if frontend_cwd is not None and not isinstance(frontend_cwd, str):
        raise YasnyError(f"run.{mode}.frontend_cwd должен быть строкой")
