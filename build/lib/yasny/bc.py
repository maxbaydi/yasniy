...
def decode_program(blob: bytes, path: str | None = None) -> ProgramBC:
    if len(blob) < len(MAGIC) + 4:
        raise YasnyError("Файл байткода слишком короткий", path=path)
    if blob[: len(MAGIC)] != MAGIC:
        raise YasnyError("Неверная сигнатура файла .ybc", path=path)
    (length,) = struct.unpack("<I", blob[len(MAGIC) : len(MAGIC) + 4])
    payload = blob[len(MAGIC) + 4 :]
    if length != len(payload):
        raise YasnyError("Некорректная длина полезной нагрузки .ybc", path=path)
    try:
        obj = json.loads(payload.decode("utf-8"))
    except Exception as exc:  # noqa: BLE001
        raise YasnyError(f"Не удалось разобрать JSON байткода: {exc}", path=path) from exc
