...
def tokenize(source: str, path: str | None = None) -> list[Token]:
    ...
    for line_no, line in enumerate(lines, start=1):
        tab_idx = line.find("\t")
        if tab_idx != -1:
            raise YasnError("Табуляция запрещена, используйте пробелы", line_no, tab_idx + 1, path)
        ...
        if not bracket_stack:
            if space_count > indent_stack[-1]:
                ...
                if space_count != indent_stack[-1]:
                    raise YasnError("Некорректный уровень отступа", line_no, 1, path)
        ...
        while idx < len(line):
            ...
            if ch in SINGLE_CHAR_TOKENS:
                if ch in OPENING_BRACKETS:
                    ...
                    if not bracket_stack:
                        raise YasnError("Лишняя закрывающая скобка", line_no, col, path)
                ...
                while idx < len(line):
                    ...
                    if escaped:
                        if cur == "n":
                            ...
                            raise YasnError(f"Неизвестная escape-последовательность: \\{cur}", line_no, idx + 1, path)
                        escaped = False
                        idx += 1
                        continue
                    if cur == "\\":
                    ...
                    raise YasnError("Незакрытая строка", line_no, col, path)
                continue
            ...
            if ch.isdigit():
                ...
                if idx < len(line) and line[idx] == ".":
                    if idx + 1 < len(line) and line[idx + 1].isdigit():
                        ...
                        raise YasnError("Ожидалась цифра после точки в числе", line_no, idx + 1, path)
                raw = line[start:idx]
                if is_float:
            ...
            raise YasnError(f"Неизвестный символ: {ch}", line_no, col, path)
    ...
    if bracket_stack:
        ...
        raise YasnError(f"Незакрытая скобка: '{open_ch}'", open_line, open_col, path)
