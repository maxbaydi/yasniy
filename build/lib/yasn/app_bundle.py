...
def read_bundle(blob: bytes, path: str | None = None) -> AppBundle:
    if len(blob) < len(APP_MAGIC) + 8:
        raise YasnError("Файл приложения слишком короткий", path=path)
    if blob[: len(APP_MAGIC)] != APP_MAGIC:
        raise YasnError("Некорректная сигнатура файла приложения", path=path)

    offset = len(APP_MAGIC)
    (meta_len,) = struct.unpack("<I", blob[offset : offset + 4])
    offset += 4
    if offset + meta_len + 4 > len(blob):
        raise YasnError("Повреждён заголовок метаданных приложения", path=path)
    ...
    try:
        ...
        raise YasnError(f"Не удалось разобрать метаданные приложения: {exc}", path=path) from exc

    (bytecode_len,) = struct.unpack("<I", blob[offset : offset + 4])
    offset += 4
    if offset + bytecode_len != len(blob):
        raise YasnError("Некорректная длина байткода в приложении", path=path)
...
def _make_windows_cmd_launcher(name: str) -> str:
    return (
        ...
        "if %ERRORLEVEL% EQU 9009 python -m yasn run-app \"%APP%\" %*\r\n"
    )
