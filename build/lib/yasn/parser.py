...
class Parser:
    ...
    def _parse_stmt(self) -> ast.Stmt:
        ...
        if self._match("="):
            ...
            raise YasnError("Левая часть присваивания должна быть переменной или индексатором", expr.line, expr.col, self.path)
    ...
    def _parse_return_stmt(self) -> ast.ReturnStmt:
        ...
        if self._check("NEWLINE"):
            raise YasnError("После 'вернуть' ожидается выражение или 'пусто'", start.line, start.col, self.path)
        value = self._parse_expr()
    ...
    def _parse_type_atom(self) -> ast.TypeNode:
        ...
        if tok.kind == "IDENT" and tok.value in PRIMITIVE_TYPE_NAMES:
            ...
            raise YasnError("Ожидался тип", tok.line, tok.col, self.path)
    ...
    def _parse_primary(self) -> ast.Expr:
        ...
        raise YasnError("Ожидалось выражение", tok.line, tok.col, self.path)
    ...
    def _expect(self, kind: str, message: str) -> Token:
        ...
        tok = self._current()
        raise YasnError(message, tok.line, tok.col, self.path)
